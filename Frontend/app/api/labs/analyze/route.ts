...
function normalizeLabName(name: string) {
  ...
  // Capitaliza solo primera letra para evitar mezclas raras por acentos.
  const titled = cleaned.charAt(0).toUpperCase() + cleaned.slice(1);

  // Acrónimos clínicos frecuentes
  return titled
...
function mentionableLab(lab: ParsedLab) {
  ...
  if (isMinorDeviation(lab)) {
    // Para variaciones mínimas, solo mencionar si son marcadores con impacto metabólico claro.
    const n = normalizeForMatch(lab.name);
...
function detectStudyType(text: string) {
  ...
  // Solo cuenta como "laboratorio" cuando hay evidencia clara de panel numérico.
...
async function extractTextFromPdf(buffer: Buffer) {
  const tempPdfPath = join(tmpdir(), `carebridge-lab-${randomUUID()}.pdf`);
  try {
    await writeFile(tempPdfPath, buffer);
    const scriptPath = join(process.cwd(), "scripts", "extract_pdf_text.py");

    const text = await new Promise<string>((resolve, reject) => {
      execFile("/Users/lb/miniconda3/bin/python3", [scriptPath, tempPdfPath], { maxBuffer: 10 * 1024 * 1024 }, (error, stdout, stderr) => {
        if (error) {
          const raw = stderr?.trim() || error.message;
          if (raw.includes("No /Root object") || raw.includes("PdfminerException")) {
            reject(new Error("No se pudo leer el PDF. Verifica que el archivo no esté dañado e inténtalo de nuevo."));
            return;
          }
...
function buildAgentResponse(text: string, notes: string): LabsAgentResponse {
  ...
  if (containsAny(lowered, ["transamin", "alt", "ast", "tgp", "tgo"])) {
    questionsForDoctor.push(
      "¿Mis transaminasas pueden estar relacionadas con medicamentos, suplementos o el tratamiento dermatológico?"
    );
  }
  ...
  while (questionsForDoctor.length < 4) {
    const fallback = [
      ...
      "¿Hay algo en estos resultados que cambie mi plan dermatológico?"
    ];
  ...
  // Hallazgos prioritarios aunque no estén "fuera de rango" tradicional
  ...
  return {
    ...
    patientSummary: [
      "Laura, ya revisé tu PDF y te traduzco lo importante en lenguaje simple.",
